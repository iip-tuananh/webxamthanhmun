...
class FrontController extends Controller
{
    ...
    public function homePage() {
        ...
        $data['courses'] = Course::query()->with(['image'])
            ...
            ->where('status', 1)
            ->orderBy('created_at', 'desc')->get();
        $data['teams'] = Team::query()->with('image')->latest()->get();
        $data['bannerRateCustomer'] = BannerPage::query()->with('image')->find(2);
    ...
    public function about_page(Request $request) {
        ...
        return view('site.about_page', compact( 'about'));
    }
    ...
    public function getListProduct(Request $request, $slug = null) {
        $category = null;
        $allCates = Category::query()->orderBy('sort_order')->get();
        ...
        return view('site.product_list', compact('products', 'category', 'allCates', 'productsLatest'));
    }
